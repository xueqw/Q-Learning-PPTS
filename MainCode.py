...
class QL_Heft:
    ...
    def read_dag_pre(self):
        ...
        filename = 'C:/Users/24682/Desktop/dag.txt'
    ...
    def read_pro_avg(self):
        ...
        self.avg_list = []
        filename = 'C:/Users/24682/Desktop/computation costs.txt'
        with open(filename, 'r') as f:
            lines = f.readlines()
            for line in lines:
                temp_sum = 0
    ...
    def get_pcm_list(self):
        num_processors = len(computation_costs[0])
        ...
        for task in range(self.task, 0, -1):  # 从最后一个任务开始向前计算
            for processor in range(num_processors):
                self.compute_pcm(task, processor, computation_costs, self.dag_relation_pre, pcm)
    ...
    def select_processor(self):
        ...
        for current_task in task_order:
            ...
            if current_task == 1:
                for i in range(self.VM):
                    EST[i] = computation_costs[current_task - 1][i]
                    ...
                    for pre in list(self.dag_relation_suc[current_task].keys()):
                        if i + 1 == list(EFT_relation[pre].keys())[0]:
                            ...
                            if i + 1 not in ava_set:
                                EST[i] = max(
                                    (EST[i], list(EFT_relation[pre].values())[0] + self.dag_relation_suc[current_task][pre]))
                            else:
                                EST[i] = max(ava_set[i + 1], max((EST[i], list(EFT_relation[pre].values())[0] +
                ...
                EFT = [0, 0, 0]
                for i in range(self.VM):
                    EFT[i] = EST[i] + computation_costs[current_task - 1][i]
                LA_eft = EFT[0] + pcm_list[current_task - 1][0]
                min_processor = 0
                for i in range(len(EST)):
                    LA_eft_temp = EST[i] + computation_costs[current_task - 1][i] + pcm_list[current_task - 1][i]
                    if LA_eft_temp <= LA_eft:
                        LA_eft = LA_eft_temp
                        VM_relation = {}
...
# 创建 QL_Heft 类的实例
ql_heft_instance = QL_Heft(9, 1, 0.8,3)

ql_heft_instance.get_order()
